...
void write_bp_chunk(
    ...
    char *result_z,
    size_t ib_bytes,
    char *work_bits_of_bp, // Must have size at least (ib_bytes * 2)
    bp_state_t *bp_state);

uint64_t process_chunk(
    char *in_buffer,
    size_t in_length,
    char *work_bits_of_d,       // Working buffer of minimum length ((in_length + 63) / 64)
    char *work_bits_of_a,       // Working buffer of minimum length ((in_length + 63) / 64)
    char *work_bits_of_z,       // Working buffer of minimum length ((in_length + 63) / 64)
    char *work_bits_of_q,       // Working buffer of minimum length ((in_length + 63) / 64)
    char *work_bits_of_b,       // Working buffer of minimum length ((in_length + 63) / 64)
    char *work_bits_of_e,       // Working buffer of minimum length ((in_length + 63) / 64)
...
int main_spliced(
  ...
  char buffer[W8_BUFFER_SIZE];

  char *bits_of_d = malloc(W32_BUFFER_SIZE); memset(bits_of_d, 0, W32_BUFFER_SIZE);
  char *bits_of_a = malloc(W32_BUFFER_SIZE); memset(bits_of_a, 0, W32_BUFFER_SIZE);
  char *bits_of_z = malloc(W32_BUFFER_SIZE); memset(bits_of_z, 0, W32_BUFFER_SIZE);
  char *bits_of_b = malloc(W32_BUFFER_SIZE); memset(bits_of_b, 0, W32_BUFFER_SIZE);
  char *bits_of_e = malloc(W32_BUFFER_SIZE); memset(bits_of_e, 0, W32_BUFFER_SIZE);
  char *bits_of_q = malloc(W32_BUFFER_SIZE); memset(bits_of_q, 0, W32_BUFFER_SIZE);

  char *work_bits_of_bp = malloc(W32_BUFFER_SIZE * 2); memset(work_bits_of_bp, 0, W32_BUFFER_SIZE * 2);

  char result_ib[W8_BUFFER_SIZE / 8];
  char result_a [W8_BUFFER_SIZE / 8];
  char result_z [W8_BUFFER_SIZE / 8];
  bp_state_t bp_state;
...
void write_bp_chunk(
  ...
  uint64_t *w64_work_bp   = (uint64_t *)work_bits_of_bp;
  ...
  for (size_t i = 0; i < w64_len; ++i) {
    ...
    // // printf("ib: "); print_bits_64(w64_ib); printf("\n");
    // // printf("a:  "); print_bits_64(w64_a ); printf("\n");
    // // printf("z:  "); print_bits_64(w64_z ); printf("\n");
    ...
    if (remainder_len + pc_ib >= 64) {
      ...
      // // printf("L: "); print_bits_64(w64_work_bp[w64s_ready]); printf("\n");
  ...
  fwrite(w64_work_bp, w64s_ready, sizeof(uint64_t), bp_out);
